...
abstract class Each extends \Lycan\Validations\Validator
{
    ...
    public function __construct($options)
    {
        $this->attributes = isset($options['attributes'])
            ? ( !is_array($options['attributes']) ? array($options['attributes']) : $options['attributes'])
            : array();

        if (empty($this->attributes))
            throw new Exception('attributes cannot be empty');

        if (isset($options['attributes'])) unset($options['attributes']);
    ...
    public function validate($record)
    {
        ...
        foreach ($this->attributes as $attribute) {
            ...
            if (null === $value && isset($this->options['allow_null']) && true == $this->options['allow_null']
                || empty($value) && isset($this->options['allow_empty']) && true == $this->options['allow_empty']
            ){continue;}
